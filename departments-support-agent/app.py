...
class DepartmentsSupportAgentHandler(BaseHTTPRequestHandler):
    server_version = "DepartmentsSupportAgentHTTP/1.0"

    def do_GET(self) -> None:
        if self.path == "/health":
            self._send_json({"status": "ok", "agent": "departments-support-agent"})
            return
...
def main() -> None:
    ...
    port = int(os.getenv("GLPI_AGENT_PORT", "8005"))
    server = ThreadingHTTPServer((host, port), DepartmentsSupportAgentHandler)
    print(f"Departments Support agent backend listening on http://{host}:{port}")
