...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--user-email", help="User email passed into the agent context")
...
def main() -> None:
    ...
    with GlpiClient(settings) as glpi:
        ...
        if args.prompt:
            print(agent.run(" ".join(args.prompt), args.user_email))
            return
        ...
        while True:
            ...
            if prompt.lower() in {"exit", "quit"}:
                break
            if prompt:
                print(agent.run(prompt, args.user_email))
