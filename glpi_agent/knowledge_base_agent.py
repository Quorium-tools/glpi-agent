...
_IT_EMAIL_PREFIXES = ("tech.", "techni", "it.", "dsi.", "admin.", "sysadmin", "helpdesk", "support.")
...
Step 0 (if user_email is in context): Call verify_non_it_user first. If is_it_staff=true or allowed=false, stop and tell the user this service is for non-IT departments only — do not continue.
...
If a tool result contains dry_run=true, explain it was a preview and nothing was changed.
"""
...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "verify_non_it_user",
            "description": "Look up a user by email in GLPI and verify they are not IT/DSI staff.",
            "parameters": {
                "type": "object",
                "properties": {
                    "user_email": {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "user_email": {
                        "type": "string",
                        "description": "Email of the user who asked.",
                    },
                },
                "required": ["query"],
            },
...
class KbAgent:
    def __init__(self, llm: OpenRouterClient, glpi: GlpiClient) -> None:
        ...
        self.tool_handlers: dict[str, Callable[..., Any]] = {
            ...
            "verify_non_it_user": self._verify_non_it_user,
            "get_ticket_by_id": self._get_ticket_by_id,
            "log_unfound_query": self._log_unfound_query,
        }

    def run(self, user_message: str, user_email: str | None = None) -> str:
    ...
    def _fallback_response(self, messages: list[dict[str, Any]]) -> str:
        ...
        if result.get("dry_run"):
            return f"(Dry-run preview) A ticket would be created for: **{result.get('title', 'your issue')}**. Nothing was sent to GLPI."
    ...
    def _requester_user_id(self) -> int | None:
        if not self._current_user_email:
            return None
        try:
            results = self.glpi.search_items(itemtype="User", filter=f"email=={self._current_user_email}", limit=1)
            if results:
    ...
    def _create_basic_ticket(
        ...
        requester_id = self._requester_user_id()
        if requester_id:
            fields["_users_id_requester"] = requester_id
        result = self.glpi.create_item("Ticket", fields)
        if result.get("dry_run"):
            return {"dry_run": True, "message": "Preview only — no ticket was sent to GLPI.", "title": title}
        return {
            "created": True,
    ...
    def _verify_non_it_user(self, user_email: str) -> dict[str, Any]:
        prefix = user_email.split("@")[0].lower()
        if any(prefix.startswith(p) for p in _IT_EMAIL_PREFIXES):
            return {
                "found": True,
                "email": user_email,
                "is_it_staff": True,
                "allowed": False,
                "reason": "Email prefix indicates IT/DSI staff account.",
            }
        try:
            results = self.glpi.search_items(itemtype="User", filter=f"email=={user_email}", limit=1)
            if not results:
                return {"found": False, "is_it_staff": False, "allowed": True, "message": "User not found in GLPI — proceeding as non-IT."}
            user = results[0]
            department = user.get("entity_name", "") or ""
            is_it = any(kw in department for kw in ("IT", "DSI", "Tech", "Informatique"))
            return {
                "found": True,
                "email": user_email,
            ...
            return {"found": False, "is_it_staff": False, "allowed": True, "message": "Could not verify user — proceeding as non-IT."}
    ...
    @staticmethod
    def _log_unfound_query(query: str, user_email: str | None = None) -> dict[str, Any]:
        return {
            "logged": True,
            "query": query,
            "user": user_email,
